...
WebSocketConnection.prototype.fragmentAndSend = function(frame, cb) {
    ...
    (function (cnt) {
        ...
        currentFrame.opcode = (cnt === 1) ? frame.opcode : 0x00;

        // fin set on last frame only
        currentFrame.fin = (cnt === numFragments);

        // length is likely to be shorter on the last fragment
        var currentLength = (cnt === numFragments) ? length - (threshold * (cnt-1)) : threshold;
        var sliceStart = threshold * (cnt-1);

        // Slice the right portion of the original payload
        currentFrame.binaryPayload = frame.binaryPayload.slice(sliceStart, sliceStart + currentLength);

        // if remaining packet then will again call this function
        var dCb = cnt++ <= numFragments ? function() { args.callee(cnt); } : sentCallback ;
        this.sendFrame(currentFrame, dCb);
    })(1);
};
